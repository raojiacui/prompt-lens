...
export async function getUserTrialUsage(userId: string) {
  ...
  const [promptRows, workflowRows] = await Promise.all([
    db.select({ count: count() }).from(analysisHistory).where(eq(analysisHistory.userId, userId)),
    db.select({ count: count() }).from(projects).where(sql`${projects.userId} = ${userId} and ${projects.status} <> 'draft'`),
  ]);

  const used = (promptRows[0]?.count || 0) + (workflowRows[0]?.count || 0);
...
export function trialQuotaResponse(error: unknown) {
  ...
  return {
    error: "你的免费视频分析额度已经用完。每个账号可以免费试用 2 次视频分析。",
