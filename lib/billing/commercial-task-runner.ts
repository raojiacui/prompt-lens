...
import { AnalysisExecutionLost, executionCondition } from "./analysis-execution";
...
export async function runCommercialTask(taskId: string) {
  const [task] = await db.update(commercialTasks).set({ state: "running", result: sql`${commercialTasks.result} || ${JSON.stringify({ executionId: randomUUID() })}::jsonb`, updatedAt: new Date() }).where(and(eq(commercialTasks.id, taskId), eq(commercialTasks.state, "queued"), sql`${commercialTasks.kind} IN ('analysis', 'generation')`)).returning();
  if (!task) return;
  try {
    ...
    await db.update(commercialTasks).set({ state: "review", result: sql`${commercialTasks.result} || '{"code":"TASK_REQUIRES_RECONCILIATION"}'::jsonb`, updatedAt: new Date() }).where(task.kind === "analysis" ? executionCondition(task) : and(eq(commercialTasks.id, task.id), eq(commercialTasks.state, "running")));
  }
}
