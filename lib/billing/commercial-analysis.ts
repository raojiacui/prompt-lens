...
export async function executeCommercialAnalysis(task: typeof commercialTasks.$inferSelect) {
  ...
  if (input.retry) {
    ...
    const result = await retrySceneAnalysis({ userId: task.userId, projectId: input.projectId, sceneVersionId: scene.sceneVersionId, modelMode: "manual", modelId: savedAnalysisModel(input), outputLanguage: input.outputLanguage, analysisApiKey: apiKey, allowPlatformKeyForAnalysis: input.pricing.payer === "platform", analysisKeySource: input.pricing.payer === "platform" ? "platform" : "user", forceFreeTrialKie: false });
...
export async function recoverCommercialAnalysisTasks() {
  const stale = await db.select().from(commercialTasks).where(and(eq(commercialTasks.kind, "analysis"), sql`${commercialTasks.state} IN ('running', 'review')`, sql`${commercialTasks.updatedAt} < now() - interval '6 minutes'`)).limit(10);
  for (const candidate of stale) {
    const [task] = await db.update(commercialTasks).set({ state: "review", updatedAt: new Date() }).where(and(eq(commercialTasks.id, candidate.id), eq(commercialTasks.updatedAt, candidate.updatedAt), sql`${commercialTasks.state} IN ('running', 'review')`)).returning();
    if (!task) continue;
    const input = task.input as AnalysisInput;
    ...
    if (!input.retry && progress.assets && progress.versionId && progress.sceneRecords) {
      for (let index = 0; index < input.pricing.scenes.length; index++) {
        ...
        const existing = await db.query.sceneVersions.findFirst({ where: and(eq(sceneVersions.projectVersionId, progress.versionId), eq(sceneVersions.originalSceneId, originalSceneId)) });
        if (!existing) await db.insert(sceneVersions).values({ projectId: input.projectId, projectVersionId: progress.versionId, originalSceneId, sceneIndex: index + 1, duration: (scene.endUs - scene.startUs) / 1_000_000, ...buildFallbackSceneBlueprint(progress.assets.scenes[index], "Analysis interrupted") });
        await db.update(videoScenes).set({ status: "failed", error: "Analysis interrupted", updatedAt: new Date() }).where(eq(videoScenes.id, originalSceneId));
      }
    }
    await finishCommercialAnalysis(task, successes, Boolean(progress.splitDelivered), successes.length > (input.retry?.previousSuccess.length || 0) ? "completed" : "failed");
  }
