...
export async function GET(request: NextRequest) {
  ...
  const orders = await db.select({ id: paymentOrders.id }).from(paymentOrders).where(and(eq(paymentOrders.provider, "xunhupay"), eq(paymentOrders.status, "pending"), sql`${paymentOrders.createdAt} > now() - interval '10 minutes'`)).orderBy(sql`COALESCE((${paymentOrders.metadata}->>'queryAfter')::bigint, 0)`, asc(paymentOrders.createdAt)).limit(3);
  let checked = 0;
  for (const order of orders) { try { await reconcilePaymentOrder(order.id); checked++; } catch { /* Keep uncertain orders pending for the next run. */ } }
  await db.update(paymentOrders).set({ metadata: sql`${paymentOrders.metadata} || '{"reconciliation":"manual_review"}'::jsonb` }).where(and(eq(paymentOrders.provider, "xunhupay"), eq(paymentOrders.status, "pending"), sql`${paymentOrders.createdAt} < now() - interval '24 hours'`));
