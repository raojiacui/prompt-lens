...
vi.mock("@/lib/payments/credit-checkout", () => ({ createAlipayCreditCheckout: vi.fn(async () => ({ orderId: "order" })) }));
...
describe("Domestic checkout route", () => {
  beforeEach(() => { vi.mocked(commercialSalesReady).mockReturnValue(false); vi.mocked(commercialAcceptanceAllowed).mockReturnValue(false); vi.mocked(createAlipayCreditCheckout).mockClear(); });
  ...
  it("rejects cross-origin checkout before creating an order", async () => {
    ...
    expect(response.status).toBe(403); expect(createAlipayCreditCheckout).not.toHaveBeenCalled();
  });
});
