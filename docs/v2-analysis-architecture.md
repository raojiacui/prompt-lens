...
- `GET /api/workflow/analysis-tasks/:id` reads progress and schedules the next queued step. Reopening an analyzing project resumes polling.
...
1. Apply migrations through `0018_workflow_analysis_tasks` before serving the updated application. `0017` adds trial reservations and shared rate limits; `0018` extends the task table and enforces one initial analysis task per project.
...
4. Verify one trial, one BYOK request, a failed request and a closed-browser task using real services before enabling public traffic. Automated tests use isolated databases and mocked provider responses; they do not verify live keys or provider availability.
